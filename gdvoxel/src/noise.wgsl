...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    if (global_id.x >= u32(settings.generated_size) || global_id.y >= u32(settings.generated_size) || global_id.z >= u32(settings.generated_size)) {
        return;
    }
    ...
    textureStore(output_noise, vec3<i32>(i32(global_id.x), i32(global_id.y), i32(global_id.z)), vec4<f32>(snoise(noise_coord), 0.0, 0.0, 1.0));
}